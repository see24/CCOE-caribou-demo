...
Table 2. Monitoring scenarios adapted from [@rettie_summary_2017]. *removed the second collar column (assuming minimum=initial) and removed collaring month and duration, but added renewal interval, and hoping to also be able to provide some sort of example of real ranges that are similar to the scenario. Anna waiting to hear back from Monitoring WG folks to get a sense of what values they've been using so that we can make these values realistic.
```{r}
tbl <- "Years|Collars|cow multiplier|collar renewal interval|example
3|30|3|1|ex range?
"
df <- read.delim(textConnection(tbl),header=FALSE,sep="|",strip.white=TRUE,stringsAsFactors=FALSE)
names(df) <- unname(as.list(df[1,]))
