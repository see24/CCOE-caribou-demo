...
*note that we've had some input on monitoring numbers from NL, BC, ON (and MB pending), so some of them may also be willing to give us case study examples to use here?
...
Table 2. Monitoring scenarios adapted from [@rettie_summary_2017]. *removed the second collar column (assuming minimum=initial) and removed collaring month and duration, but added renewal interval, and hoping to also be able to provide some sort of example of real ranges that are similar to the scenario. 
