...
In order to assess whether a monitoring program is likely sufficient to inform decisions, it is important to select outcome measures that are relevant to decision making [@gregory_structured_2012]. For boreal caribou and other endangered species, it is important to know whether a population is declining or not. If uncertainty about population status is low, additional investment in reducing uncertainty about the population growth rate may not be necessary or warranted. Thus, in this analysis our outcome of interest is how often the Bayesian posterior mean correctly predicts the true population status (increasing or decreasing). To calculate this outcome for each disturbance and monitoring scenario (Figure \@ref(fig:figDisturbance)) we fit the Bayesian model to 200 example population trajectories (e.g. Figure \@ref(fig:exampleTrajectories)). The probability of correct status assessment for that scenario is the percentage of these 200 examples in which the Bayesian posterior mean population growth rate and the true population growth rate are both above or below 1. 
...
Include some commentary on addressing limitations of aspatial demographic models with spatial models. Not a panacea. It is possible (cite examples) to develop spatially explicit models of movement and adult mortality risk - though sample size is a challenge. Recruitment, which is more variable among ranges and with anthropogenic disturbance, is difficult to characterize at finer spatial scales because it depends on the conditions experienced by mothers over years as they move across the range, and on spatial variation in calf mortality risk that is not measured directly. 
...
Re paragraph on costs of monitoring. The reality is that costs vary with remoteness and the difficulty of finding animals to collar (which also depends on how many collars are deployed). Since costs are so context-specific, there isn't a good way to incorporate them into this general analysis. It would be interesting to work through a case study (i.e. Newfoundland) informed by context-specific costs, and consider how incorporation of those costs may change the relative value of alternative monitoring scenarios.

Biases in composition survey estimates also vary with vegetation cover (i.e. the difficulty of seeing animals) and the care taken to correctly identify the sex of unknown animals. Misidentification rates could be estimated from discrepancies among results from multiple independent observers analyzing photos of animal groups.

The tool we have developed is a proof of concept, designed to demonstrate possibilities and help users develop a general understanding of the relative value of alternative monitoring strategies for boreal caribou. In current form the tool will have limited value for managers who have some local data and wish to know the value of collecting more, because it is designed to either integrate real local data or simulated local data, but not both. Development of a tool designed to help decision makers assess the relative value of strategies for additional monitoring given their existing local data and national-demographic disturbance relationships would be a useful followup to this work. 

A tool that integrates real local data with national demographic disturbance relationships can also be used to provide consistent and transparent assessments of the strength of evidence for local deviations from the expected mean of national demographic-disturbance relationships. Help distinguish real variation among populations from apparent variation that can be attributed to lack of local data.
