...
runScenarios = F 
doPriorCalibration=T #Note the prior calibration is time consuming. 
```
...
Estimated recruitment probability is adjusted for sex ratio to get expected recruits per female: $X_t=R_t/2$. We include an option to also adjust for delayed age of first reproduction [@eacker_web-based_2019;@decesare_estimating_2012], but to maintain consistency with Johnson et al. [-@johnson_science_2020] we do not apply this adjustment by default: $$X_t=\frac{R_t/2}{1+R_t/2}.$$ 
...
To simulate plausible plausible demographic trajectories for example populations we used a national model described by Dyson et al. [-@dyson_existing_2022] and Stewart et al. [-@stewart_climate-informed_2023]. Expected recruitment ($\bar{R}_t$) and survival ($\bar{S}_t$) vary with disturbance according the beta regression models estimated by Johnson et al. [@johnson_science_2020]:  :$$\bar{R}_t~Beta(\mu^R_t,\phi^R);log(\mu^R_t)=\dot{\beta^R_0}+\dot{\beta^R_a}A_t+\dot{\beta}^R_fF_t,$$
$$\bar{S}_t~(46\times Beta(\mu^S_t,\phi^S)-0.5)/45;log(\mu^S_t)=\dot{\beta^S_0}+\dot{\beta^S_a}A_t.$$
$\phi^R~Normal(19.862,2.229)$ and $\phi^S~Normal(63.733,8.311)$ are precisions of the Beta distributed errors [@ferrari_beta_2004]. At the beginning of a simulation for an example population, regression coefficient values are sampled from Gaussian distributions (see Table \@ref(tab:priors)) and the population is assigned to quantiles of the Beta error distributions for survival and recruitment. The population remains in these quantiles as disturbance changes over time, so there is substantial persistent variation in recruitment and survival among example among populations (example figure). There is also interannual variation in recruitment for each population, modelled using truncated beta distributions (rtrunc function [@novomestky_package_2016]): $$\dot{R}_t~$$
...
Table A1. Variables and parameters in the Bayesian integrated population model and the national demographic model.
