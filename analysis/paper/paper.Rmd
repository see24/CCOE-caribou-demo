---
title: "National demographic-disturbance relationships and local monitoring both reduce uncertainty in projections of boreal caribou population viability: A Bayesian integrated population model, demographic simulations and a Shiny interface show how"
author:
  - name: Josie Hughes
...
# Abstract
Across fifty-eight boreal caribou ranges in Canada, both survival and recruitment decrease with the percentage of the range that is within five hundred metres of anthropogenic disturbance. However, there is also variation in demographic rates among ranges particularly where anthropogenic disturbance is low, and this variation leads to uncertainty in demographic projections derived from national disturbance-demographic relationships. Demographic projections derived from local range-specific demographic data are also uncertain because few places have enough long-term monitoring data to characterize local demographic-disturbance relationships. We present a simple Bayesian integrated population model to integrate prior information from national analysis of demographic-disturbance relationships with available local demographic data to reduce uncertainty in population viability projections. We combine this model with methods for simulating local population dynamics and monitoring to show how monitoring requirements depend on landscape condition. Where projected anthropogenic disturbance is very high or very low, reasonably accurate status projections can be made using only national demographic-disturbance relationships. Where uncertainty is higher, deploying few collars for more years may be a lower cost option for improving the reliability of status projections. We provide an R Shiny interface to enable others to explore the implications of alternative monitoring scenarios, disturbance trajectories, priors, and other model parameters for boreal caribou demographic projections.
...
```{r fig2, fig.cap="Power analysis example result.", cache=TRUE}
