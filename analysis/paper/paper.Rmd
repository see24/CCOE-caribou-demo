...
## Integration of local demographic data and national disturbance-demographic relationships in Bayesian population model

Following Eacker et al. [-@eacker_web-based_2019], the observed number of calves $\hat{C}_t$ is a binomially distributed function of the observed number of adult female caribou $\hat{W}_t$ and the estimated recruitment rate $R_t$: $$\hat{C}_t \sim \text{Binomial}(\hat{W}_t,R_t).$$
...
Estimated recruitment probability is adjusted for sex ratio to get expected recruits per female: $\tilde{R}_t=R_t/2$. We include an option to also adjust for delayed age of first reproduction [@eacker_web-based_2019;@decesare_estimating_2012], but to maintain consistency with Johnson et al. [-@johnson_science_2020] we do not apply this adjustment by default: $$\tilde{R}_t=\frac{R_t/2}{1+R_t/2}.$$ 
...
All regression coefficients are assumed to be Gaussian distributed, with priors derived from the expected values and 95% confidence intervals estimated by Johnson et al (2020) (Table \@ref(tab:priors)). We calibrated the prior distributions so that the 95% prediction interval from the IPM matches the range between the 2.5% and 97.5% quantiles of 1000 simulated recruitment (Appendix A).  
...
We assume a simple two-class demographic model, with a census in late winter of surviving females and new recruits to the post-juvenile (aged 1+) class. The number of post-juvenile females that survive from year $t$ to the census $A_t$ is binomially distributed with survival probability $S_t$: $A_{t} \sim \text{Binomial}(N_t,S_t)$. The number of juveniles recruiting to the post-juvenile class at the census is a Poisson distributed function of the number of surviving post-juvenile females and the adjusted recruitment rate $\tilde{R}_t$: $J_{t} \sim \text{Poisson}(\tilde{R}_tA_t)$. The post-juvenile female population in the next year includes both survivors and new recruits: $N_{t+1}=A_t+J_t$. Annual population growth rate $\lambda_t$ is set to 0 when $N_t$ is 0, and is otherwise $\lambda_t=N_{t+1}/N_t$. For more stable estimates of population status, we report average population growth over a period of $y$ years, calculated as $\bar{\lambda}_{y,t}=\sum_{x=t-y+1}^{x=t}\lambda_x$. The initial post-juvenile population size $N_0$ is 1000 animals. For reference, all variables and parameters are defined in Table A1. 

<!--
Annotated equations for poster.
$$\underbrace{N_{t+1}}_{\text{Adult females in year t+1}}=\underbrace{A_t}_{\text{Surviving adults}}+\underbrace{J_t}_{\text{New recruits to adult age class}}$$
$$A_{t} \sim \text{Binomial}(N_t,\underbrace{S_t}_{\text{Adult survival probability}})$$
...
$$J_{t} \sim \text{Poisson}(A_t\underbrace{R_t/2}_{\text{Recruitment rate}})$$
...
## A method for simulating example local demographic data
...
```{r fig1, fig.cap="Anthropogenic disturbance and monitoring scenarios.", cache=TRUE}
knitr::include_graphics(here::here("figs/forPaper/distScnscmult3ay3aSf1.png"))
...
```{r fig2, fig.cap="Example projections, assuming a target of 30 collars, renewed every 4 years, with 3 cows per collared cow included in aerial surveys, and a 1% increase in anthropogenic disturbance per year (as in Figure 1). The number of monitoring years varies from P=0 (top row) to P=24 (bottom row), and the anthropogenic disturbance footprint in 2023 varies from 0% (left column) to 60% (right column). A status assessment error occurs when the projected population growth rate is above one and the true rate is below one or vice versa. When disturbance is very high (e.g. Anthro2023 = 60) populations that differ substantially from one another (compare 'true' lines for high and low quantile populations) share the same status, and a projection informed only by national demographic disturbance relationships (P=0) is accurate for both. When disturbance is intermediate, populations that differ substantially from one another are more likely to differ in their status. In these cases, integrating local monitoring data improves projection accuracy (i.e. smaller distances between projected and true lines) and reduces status assessment errors.", cache=TRUE}
knitr::include_graphics(here::here("figs/forPaper/examples.png"))
```

```{r fig3, fig.cap="Power analysis example result.", cache=TRUE}
knitr::include_graphics(here::here("figs/forPaper/powercmult3ay3aSf1.png"))
knitr::include_graphics(here::here("figs/forPaper/powerEffortcmult3ay3aSf1.png"))

#TO DO: figure out how to put no monitoring results on these graphs.
```
...
# Appendix
##Definitions of variables and parameters in the Bayesian integrated population model

Table A1. Variables and parameters in the Bayesian integrated population model.
...
|$y$|years|Assessment period for population growth rate.|
|$N_0$|animals|Initial adult population size.|
|**Observed**|
|$a_t$|%|Buffered anthropogenic disturbance.|
|$f_t$|%|Fire disturbance.| 
|$\hat{C}_t$|animals|Calves observed in calf:cow survey.|
...
|**Other state**|
|$\tilde{R}_t$|rate|Recruitment adjusted for sex ratio and (optionally) delayed age of first reproduction.|
|$A_t$|animals|Surviving adult females before recruitment.|
